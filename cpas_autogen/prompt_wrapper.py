...
"""Utilities for wrapping prompts with seed token metadata and signatures."""

import hashlib
import json


def generate_signature(seed_token: dict, prompt: str) -> str:
    """Return SHA-256 signature for ``seed_token`` and ``prompt``."""
    sha = hashlib.sha256()
...
def wrap_with_seed_token(prompt: str, seed_token: dict) -> str:
    ...
    signature = generate_signature(seed_token, prompt)
    header_lines = [
        "### Seed Instance",
...
__all__ = ["wrap_with_seed_token", "generate_signature"]
